	...
	?>	
		<div class="row">
			<div class="col-md-12">
				...
				<h3>Gestión de Examen Físico</h3>
				<form class="form-horizontal" method="post" id="anadirexamen" action="index.php?view=anadirexamen&id=<?php echo $historia->hisIdHistoria; ?>" role="form">
				...
				<?php
				$deta = array(); 
				$deta = SPDetConceptos::getAllId(19);
				//$historia = array(); $historia = SPHistoria::getByPac($_GET["id"]);
				$dethistoria = array(); $dethistoria = SPDetHistoria::getByHist($historia->hisIdHistoria,19);
				$con = 0;
				if(count($deta)>0){
					// si hay datos
					?>
					...
					<?php foreach($deta as $det) { 
						$con = $con + 1; 
						$dethistoria = array(); $dethistoria = SPDetHistoria::getByIdHist($historia->hisIdHistoria,19,$det->detIdDetalle); 
