	...
	?>	
		<div class="row">
			<div class="col-md-12">
				...
				<h3>Gestión de Antecedentes Personales Hábitos</h3>
				<form class="form-horizontal" method="post" id="anadirhabitos" action="index.php?view=anadirhabitos&id=<?php echo $historia->hisIdHistoria; ?>" role="form">
				...
				<?php
				$deta = array(); 
				$deta = SPDetConceptos::getAllId(26);
				//$historia = array(); $historia = SPHistoria::getByPac($_GET["id"]);
				$dethistoria = array(); $dethistoria = SPDetHistoria::getByHist($historia->hisIdHistoria,26);
				$con = 0;
				if(count($deta)>0){
					// si hay datos
					?>
					...
					<?php foreach($deta as $det) { 
						$con = $con + 1; 
						$dethistoria = array(); $dethistoria = SPDetHistoria::getByIdHist($historia->hisIdHistoria,26,$det->detIdDetalle); 
