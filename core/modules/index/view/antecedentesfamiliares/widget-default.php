	...
	?>	
		<div class="row">
			<div class="col-md-12">
				...
				<h3>Gestión de Antecedentes Familiares</h3>
				<form class="form-horizontal" method="post" id="anadirantfam" action="index.php?view=anadirantfam&id=<?php echo $historia->hisIdHistoria; ?>" role="form">
				...
				<?php
				$deta = array(); 
				$deta = SPDetConceptos::getAllId(16);
				//$historia = array(); $historia = SPHistoria::getByPac($_GET["id"]);
				$dethistoria = array(); $dethistoria = SPDetHistoria::getByHist($historia->hisIdHistoria,16);
				$con = 0;
				if(count($deta)>0){
					// si hay datos
					?>
					...
					<?php foreach($deta as $det) { 
						$con = $con + 1; 
						$dethistoria = array(); $dethistoria = SPDetHistoria::getByIdHist($historia->hisIdHistoria,16,$det->detIdDetalle); 
