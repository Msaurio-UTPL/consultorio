	...
	?>	
		<div class="row">
			<div class="col-md-12">
				...
				<h3>Gestión de Antecedentes Personales Pediátricos</h3>
				<form class="form-horizontal" method="post" id="anadirpediatricos" action="index.php?view=anadirpediatricos&id=<?php echo $historia->hisIdHistoria; ?>" role="form">
				...
				<?php
				$deta = array(); 
				$deta = SPDetConceptos::getAllId(25);
				//$historia = array(); $historia = SPHistoria::getByPac($_GET["id"]);
				$dethistoria = array(); $dethistoria = SPDetHistoria::getByHist($historia->hisIdHistoria,25);
				$con = 0;
				if(count($deta)>0){
					// si hay datos
					?>
					...
					<?php foreach($deta as $det) { 
						$con = $con + 1; 
						$dethistoria = array(); $dethistoria = SPDetHistoria::getByIdHist($historia->hisIdHistoria,25,$det->detIdDetalle); 
