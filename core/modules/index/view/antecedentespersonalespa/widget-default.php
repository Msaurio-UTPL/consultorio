	...
	?>	
		<div class="row">
			<div class="col-md-12">
				...
				<h3>Gestión de Antecedentes Personales Patológicos</h3>
				<form class="form-horizontal" method="post" id="anadirpatologicos" action="index.php?view=anadirpatologicos&id=<?php echo $historia->hisIdHistoria; ?>" role="form">
				...
				<?php
				$deta = array(); 
				$deta = SPDetConceptos::getAllId(24);
				//$historia = array(); $historia = SPHistoria::getByPac($_GET["id"]);
				$dethistoria = array(); $dethistoria = SPDetHistoria::getByHist($historia->hisIdHistoria,24);
				$con = 0;
				if(count($deta)>0){
					// si hay datos
					?>
					...
					<?php foreach($deta as $det) { 
						$con = $con + 1; 
						$dethistoria = array(); $dethistoria = SPDetHistoria::getByIdHist($historia->hisIdHistoria,24,$det->detIdDetalle); 
